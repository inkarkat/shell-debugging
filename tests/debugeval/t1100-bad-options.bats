...
@test "multiple COMMANDLINEs / COMMANDLINE + SIMPLECOMMAND cause error" {
    export DEBUG=another

    run -2 --separate-stderr debugeval --command "${DEBUG:-echo}" foo --command bar
    assert_output ''
    output="$stderr" assert_output 'ERROR: COMMAND is allowed only once, but given "foo" and "bar" (minus "another").'

    run -2 --separate-stderr debugeval --command "${DEBUG:-echo}" foo -- simple-bar
    assert_output ''
